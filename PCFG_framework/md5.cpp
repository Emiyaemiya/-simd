...
void MD5Hash_NEON(string inputs[], bit32 states[][4]) {
    ...
    uint32x4_t state3 = vdupq_n_u32(0x10325476);
        ...
        for (int k = 0; k < 16; ++k) {
            uint32_t x_buf[4] = {0};
            for (int j = 0; j < 4; ++j) {
                x_buf[j] = *reinterpret_cast<uint32_t*>(paddedMessages[j] + k * 4);
            }
            x[k] = vld1q_u32(x_buf);
        }
...
void MD5Hash_NEON_2(string inputs[], bit32 states[][4]) {
    const int parallel_factor = 2; // 改为同时处理2个消息
    uint32x4_t state0 = vcombine_u32(vdup_n_u32(0x67452301), vdup_n_u32(0x67452301)); 
    uint32x4_t state1 = vcombine_u32(vdup_n_u32(0xefcdab89), vdup_n_u32(0xefcdab89));
    uint32x4_t state2 = vcombine_u32(vdup_n_u32(0x98badcfe), vdup_n_u32(0x98badcfe));
    uint32x4_t state3 = vcombine_u32(vdup_n_u32(0x10325476), vdup_n_u32(0x10325476));

    Byte* paddedMessages[parallel_factor];
    int lengths[2] = {0}; // 调整为2
    ...
    uint32x4_t x[16];
    for (int k = 0; k < 16; ++k) {
        uint32_t x_buf[4] = {0}; // 前两个有效，后两个置零
        for (int j = 0; j < 2; ++j) {
            memcpy(&x_buf[j], paddedMessages[j] + k * 4, 4);
        }
        x[k] = vld1q_u32(x_buf);
    }
    ...
    // 结果存储（仅处理前两个结果）
    bit32 state0_arr[4], state1_arr[4], state2_arr[4], state3_arr[4];
    vst1q_u32(state0_arr, state0);
    vst1q_u32(state1_arr, state1);
    vst1q_u32(state2_arr, state2);
    vst1q_u32(state3_arr, state3);

    for (int i = 0; i < 2; ++i) { // 只处理有效输入
        states[i][0] = __builtin_bswap32(state0_arr[i]);
        states[i][1] = __builtin_bswap32(state1_arr[i]);
        states[i][2] = __builtin_bswap32(state2_arr[i]);
        states[i][3] = __builtin_bswap32(state3_arr[i]);
