...
//g++ main3.cpp train.cpp guessing.cpp md5.cpp  -o main3 -O2
// 修改后的主函数
int main() {
    double time_hash = 0;  
    double time_guess = 0; 
        ...
        if (curr_num > 1000000) {
            ...
            while (processed < q.guesses.size()) {
                ...
                if (batch_buffer.size() == SIMD_BATCH_SIZE) {
                    ...
                    MD5Hash_NEON_2(batch_inputs, batch_states);
                    for (int i = 0; i < 2; ++i) {
                        cout << batch_buffer[i] << "\t";
                        for (int j = 0; j < 4; ++j) {
...
/*
-O2
Guess time: 0.737203 seconds
Hash time: 3.11454 seconds
Train time: 33.5554 seconds
*/